...
import { PiggyBank, PlusCircle, Hammer, UserPlus } from "lucide-react";
...
export default function ChitFunds() {
  ...
  const handlePayInstallment = async (instId) => {
    ...
    try {
      setActionLoading(true);
      await axios.post(
        `/api/chits/${selectedGroup.id}/installment/${instId}/pay`,
        {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 fade-in">
      {/* Listing Panel */}
      <div
        className={`xl:col-span-8 bg-card border border-border rounded-2xl p-6 shadow-sm space-y-6 ${
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Manage mutual savings pools, register subscribers, and resolve
              monthly bidding discounts.
            </p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {groups.map((group) => (
              <div
                key={group.id}
                className="border border-border hover:border-primary/50 transition-colors p-5 rounded-xl space-y-4 relative overflow-hidden group"
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      Term: {group.durationMonths} Mos &bull; Base Contribution:
                      Rs. {group.monthlyContribution.toLocaleString()}/Mo
                    </span>
                  </div>
                  <span
      ...
      {selectedGroup && (
        <div className="xl:col-span-4 bg-card border border-border rounded-2xl p-6 shadow-sm space-y-6 fade-in">
          ...
          <div className="space-y-2.5">
            <h4 className="font-bold text-[10px] text-muted-foreground uppercase tracking-widest border-l-2 border-primary pl-2">
              Auctions History
            </h4>
            {selectedGroup.auctions.length === 0 ? (
              <p className="text-[10px] text-muted-foreground">
                No auctions held yet. Conduct first auction installment.
              </p>
            ) : (
              <div className="max-h-40 overflow-y-auto space-y-2 pr-1">
                {selectedGroup.auctions.map((a) => (
                  <div
                    key={a.id}
                    className="border border-border p-2.5 rounded-lg text-[10px] space-y-1 bg-accent/10"
                  >
                    <div className="flex justify-between">
                      <span className="font-bold">
                        Round #{a.installmentNo} Winner
                      </span>
                      <span className="font-semibold text-primary">
                        {a.winningBidder.user.fullName}
                      </span>
                    </div>
          ...
          <div className="space-y-2.5">
            ...
            {selectedGroup.installments.length === 0 ? (
              <p className="text-[10px] text-muted-foreground">
                No contributions due at this stage.
              </p>
            ) : (
              <div className="max-h-56 overflow-y-auto space-y-2.5 pr-1">
                {selectedGroup.installments.map((inst) => (
                  <div
                    key={inst.id}
                    className="border border-border p-3 rounded-lg text-[10px] space-y-2"
                  >
                    ...
                    {inst.status !== "PAID" && (
                      <div className="flex items-center gap-2 pt-1 border-t border-border">
                        <input
                          ...
                          onChange={(e) =>
                            setInstallmentPayAmount(e.target.value)
                          }
            ...
            <form onSubmit={handleJoinGroup} className="space-y-4 text-xs">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setJoinForm({ ...joinForm, customerId: e.target.value })
                  }
                  required
                  className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                >
              ...
              <div>
                <label className="font-bold text-muted-foreground block mb-1">
                  Assign Slot Number (1 - {selectedGroup.maxMembers})
                </label>
                <input
                  type="number"
                  min="1"
                  ...
                  onChange={(e) =>
                    setJoinForm({ ...joinForm, slotNumber: e.target.value })
                  }
                  className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                />
              </div>
          ...
          <div className="bg-card border border-border rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl fade-in">
            ...
            <form onSubmit={handleConductAuction} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAuctionForm({
                      ...auctionForm,
                      winningBidderId: e.target.value,
                    })
                  }
                  required
                  className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                >
                  ...
                  {selectedGroup.members.map((m) => (
                    <option key={m.customerId} value={m.customerId}>
                      Slot #{m.slotNumber} - {m.customer.user.fullName}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="bg-slate-900 text-slate-300 p-3 rounded-lg text-[10px] space-y-1">
                ...
                <p>
                  Admin Fee (5%): Rs.{" "}
                  {(selectedGroup.groupValue * 0.05).toLocaleString()}
                </p>
                <p>
                  Dividend Pool: Rs.{" "}
                  {Math.max(
                    0,
                    parseFloat(auctionForm.bidAmount || "0") -
                      selectedGroup.groupValue * 0.05,
                  ).toLocaleString()}
                </p>
                <p>
                  Prize Paid to Winner: Rs.{" "}
                  {(
