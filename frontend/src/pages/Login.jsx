  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isForgot) {
        ...
        setInfo(
          "If the account exists, a recovery link has been dispatched to your email.",
        );
        setIsForgot(false);
      } else if (isRegister) {
        // Handle register
      ...
      setError(
        err.response?.data?.error ||
          "Authentication attempt failed. Please check inputs.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="hidden lg:flex lg:col-span-7 bg-[#0b192c] flex-col justify-between p-12 border-r border-slate-900 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 bg-slate-950/20 p-4 rounded-2xl border border-slate-900 w-fit">
          <img
            ...
            className="h-16 rounded-xl object-contain bg-white p-1"
          />
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-medium tracking-widest uppercase">
              Trust &bull; Growth &bull; Together
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6 max-w-lg">
          ...
          <p className="text-slate-400 text-sm leading-relaxed">
            Finance management software tailored for chit funds, local credit
            lenders, and LIC insurance agencies. Powered by predictive AI risk
            profiling.
          </p>
        </div>

        <div className="text-xs text-slate-500">
          &copy; {new Date().getFullYear()} Swaraj Pvt. Limited. All rights
          reserved. Registered under NBFC guidelines.
        </div>
      </div>
        ...
        <div className="w-full max-w-md mx-auto space-y-8">
          ...
          <div className="lg:hidden flex items-center gap-3 mb-6 bg-[#0b192c] p-3 rounded-xl border border-slate-900">
            <img
              src="/logo.jpg"
              alt="Swaraj Private Limited Logo"
              className="h-10 rounded-lg bg-white p-0.5 object-contain"
            />
            <div>
              ...
              <>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value={UserRole.AGENT}>
                      LIC Agent / Field Broker
                    </option>
          ...
          {!isForgot && !isRegister && (
            <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800/80 text-[11px] text-slate-400 leading-normal space-y-1">
              <span className="font-bold text-white text-xs block mb-1">
                💡 Sandbox Admin & Backoffice Credentials:
              </span>
              <p>
                Admin:{" "}
                <code className="text-primary">admin@swarajfinance.com</code> /{" "}
                <code className="text-primary">password123</code>
              </p>
              <p>
                Super Admin:{" "}
                <code className="text-primary">
                  superadmin@swarajfinance.com
                </code>{" "}
                / <code className="text-primary">password123</code>
              </p>
              <p>
                Customer:{" "}
                <code className="text-primary">customer1@gmail.com</code> /{" "}
                <code className="text-primary">password123</code>
              </p>
            </div>
          )}
