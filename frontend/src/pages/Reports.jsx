...
export default function Reports() {
  ...
  const handleSendReminders = async () => {
    try {
      ...
      alert(
        `Overdue scan complete! Dispatch successful. Sent ${res.data.count} due reminders (email and WhatsApp logs registered).`,
      );
    } catch (err) {
  ...
  // Downloads helper
  const triggerDownload = async (reportPath, extraParams = {}) => {
    try {
      ...
      link.setAttribute(
        "download",
        `${reportPath}_report_${new Date().toISOString().split("T")[0]}.${ext}`,
      );
  ...
  const reportsList = [
    {
      ...
      description:
        "Directory list of registered subscribers, Aadhaar/PAN, monthly income, and KYC statuses.",
      icon: Users,
    ...
    },
    {
      ...
      description:
        "Lending summary including principal values, outstanding balances, terms, and statuses.",
      icon: Banknote,
    ...
    },
    {
      ...
      description:
        "Active savings group holdings, member counts, durations, and auctions completed.",
      icon: PiggyBank,
    ...
    },
    {
      ...
      description:
        "Insurance summary of registered plans, agent commissions, sum assured, and premiums.",
      icon: HeartHandshake,
    ...
    },
    {
      ...
      description:
        "Critical collections checklist detailing clients with pending EMI or Chit payments past due.",
      icon: AlertTriangle,
      color: "bg-red-500/10 text-red-500",
      path: "defaulters",
      filters: (
        <p className="text-[10px] text-muted-foreground leading-normal mt-2">
          Outputs list sorted by days past due. Filters not required.
        </p>
      ),
      getParams: () => ({}),
    },
    {
      ...
      description:
        "Consolidated ledger statement tracking interest margins, commission revenues, and loan capital outflow.",
      icon: TrendingUp,
      color: "bg-purple-500/10 text-purple-500",
      path: "cash-flow",
      filters: (
        <p className="text-[10px] text-muted-foreground leading-normal mt-2">
          Calculates net ledger balance sheets. Filters not required.
        </p>
      ),
      getParams: () => ({}),
    },
    {
      ...
      description:
        "Unified payment audit details showing Loan, Chit, and LIC users who paid or have unpaid dues.",
      icon: FileSpreadsheet,
      color: "bg-amber-500/10 text-amber-500",
      path: "monthly-status",
      filters: (
        <p className="text-[10px] text-muted-foreground leading-normal mt-2">
          Consolidates collections status. Driven by Global Date Filter.
        </p>
      ),
      getParams: () => ({}),
    },
  ...
  return (
    <div className="space-y-6 fade-in text-xs sm:text-sm">
      ...
      <div className="flex justify-between items-center flex-wrap gap-4 border-b border-border pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Download audit-compliant spreadsheets, structured PDFs, or CSV data
            files with Swaraj Pvt. Limited branding.
          </p>
        </div>
        <button
        ...
        >
          ...
          <span>
            {sendingReminders ? "Dispatching..." : "Dispatch Due Reminders"}
          </span>
        </button>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-3">
        <h3 className="font-extrabold text-xs uppercase tracking-wider text-muted-foreground border-l-2 border-primary pl-2">
          Global Date Filter
        </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reportsList.map((report) => (
          <div
            key={report.id}
            className="bg-card border border-border rounded-2xl p-5 shadow-sm flex flex-col justify-between hover:border-primary/50 transition-colors space-y-4"
          >
