...
import { Banknote, Search, PlusCircle } from "lucide-react";
import { LoanType } from "../../../shared/src/types";
...
export default function Loans() {
  ...
  const handleVerifyCollateral = async (status) => {
    if (!selectedLoan) return;
    try {
      setActionLoading(true);
      await axios.post(`/api/loans/${selectedLoan.id}/verify-collateral`, {
        status,
      });
      await reloadSelectedLoan();
      ...
      alert(
        err.response?.data?.error || "Failed to resolve collateral assessment.",
      );
    } finally {
      setActionLoading(false);
    }
  ...
  const reloadSelectedLoan = async () => {
    ...
    if (selectedLoan) {
      const res = await axios.get("/api/loans");
      const updated = res.data.find((l) => l.id === selectedLoan.id);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Manage active accounts, track monthly EMIs, and process
              part-payments.
            </p>
          </div>
          <button
          ...
          <div className="bg-accent/30 p-4 rounded-xl space-y-2">
            <div className="flex justify-between text-xs">
              ...
              <span className="font-bold">
                {selectedLoan.customer.user.fullName}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Principal Value</span>
              <span className="font-semibold">
                Rs. {selectedLoan.principal.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Outstanding Balance</span>
              <span className="font-extrabold text-primary">
                Rs. {selectedLoan.outstandingBalance.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              ...
              <span
                className={`font-bold px-1.5 py-0.5 rounded text-[9px] uppercase ${
                  selectedLoan.collateralStatus === "VERIFIED"
                    ? "bg-green-500/10 text-green-500"
                    : selectedLoan.collateralStatus === "REJECTED"
                      ? "bg-red-500/10 text-red-500"
                      : "bg-yellow-500/10 text-yellow-500"
                }`}
              >
                {selectedLoan.collateralStatus}
              </span>
            </div>
              ...
              <div className="text-[10px] text-muted-foreground bg-accent/40 p-2.5 rounded-lg space-y-1 mt-2">
                ...
                {selectedLoan.collateralUrl && (
                  <p className="truncate text-[9px] text-primary mt-1">
                    Doc:{" "}
                    <a
                      href={selectedLoan.collateralUrl}
                      target="_blank"
                      rel="noreferrer"
            ...
            <div className="space-y-4">
              ...
              {selectedLoan.collateralStatus !== "VERIFIED" ? (
                <div className="border border-yellow-500/20 bg-yellow-500/5 p-3 rounded-xl space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-bold text-yellow-500/90">
                      Collateral Audit Pending
                    </span>
                    <span className="text-[9px] bg-yellow-500/15 text-yellow-500 px-1.5 py-0.5 rounded font-extrabold">
                      {selectedLoan.collateralStatus}
                    </span>
                  </div>
                  <p className="text-[9px] text-muted-foreground leading-normal">
                    You must verify the collateral asset details and document
                    authenticity before sanctioning/approving the credit file.
                  </p>
              ...
              <div className="flex gap-2 border-t border-border pt-3">
                <button
                  ...
                  title={
                    selectedLoan.collateralStatus !== "VERIFIED"
                      ? "Verify collateral to enable approval"
                      : "Sanction Loan File"
                  }
                >
                  Sanction / Approve
          ...
          {selectedLoan.status === "DISBURSED" && (
            <div className="space-y-4">
              ...
              <div className="max-h-72 overflow-y-auto space-y-3 pr-1">
                {selectedLoan.emiInstallments.map((emi) => (
                  <div
                    key={emi.id}
                    className="border border-border p-3 rounded-lg text-xs space-y-2"
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-bold">
                        EMI #{emi.installmentNo}
                      </span>
                      <span
                    ...
                    <div className="flex justify-between text-[10px] text-muted-foreground">
                      ...
                      <span>
                        Principal: Rs. {emi.principalAmount} &bull; Interest:
                        Rs. {emi.interestAmount}
                      </span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl fade-in max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleApplySubmit} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, tenorMonths: e.target.value })
                    }
                    className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                  />
                </div>
              ...
              <div>
                <label className="font-bold text-muted-foreground block mb-1">
                  Collateral Document Drive URL / Link
                </label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    setForm({ ...form, collateralUrl: e.target.value })
                  }
                  className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                />
              </div>
