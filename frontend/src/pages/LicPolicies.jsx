...
import { HeartHandshake, PlusCircle, AlertTriangle } from "lucide-react";
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-xl font-extrabold tracking-tight">
              LIC Policies Registry
            </h2>
            <p className="text-xs text-muted-foreground">
              Register new coverage, check upcoming dues renewals, and trace
              commissions logs.
            </p>
          </div>
          <button
          ...
          <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-xl flex items-start gap-3 text-orange-500 text-xs">
            ...
            <div>
              <p className="font-bold">
                Renewal Alerts: {alerts.length} Policy Premiums Expiring
              </p>
              <p className="text-[10px] text-muted-foreground mt-0.5">
                The following policies are due within the next 30 days. Send
                WhatsApp automated notifications to subscribers.
              </p>
            </div>
          </div>
        )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 text-right font-semibold">
                        Rs. {p.premiumAmount.toLocaleString()}{" "}
                      ...
                      <td className="py-4 text-center">
                        <span
                          className={`font-semibold ${isDueSoon ? "text-orange-500 font-bold" : ""}`}
                        >
          ...
          <div className="bg-accent/30 p-4 rounded-xl space-y-2 text-xs">
            <div className="flex justify-between">
              ...
              <span className="font-bold">
                {selectedPolicy.customer?.user?.fullName}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Sum Assured</span>
              <span className="font-semibold">
                Rs. {selectedPolicy.sumAssured?.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Premium Value</span>
              <span className="font-extrabold text-primary">
                Rs. {selectedPolicy.premiumAmount?.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Billing Cycle</span>
              <span className="font-semibold">
                {selectedPolicy.premiumMode}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Next Due Date</span>
              <span className="font-semibold text-orange-500">
                {new Date(selectedPolicy.dueDate).toLocaleDateString()}
              </span>
            </div>
            <div className="flex justify-between border-t border-border pt-1">
              <span className="text-muted-foreground">Field Agent</span>
              <span className="font-semibold">
                {selectedPolicy.agent?.fullName || "Direct Sale"}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Comm rate</span>
              <span className="font-semibold text-green-600">
                {selectedPolicy.commissionRate}% commission
              </span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            {selectedPolicy.premiumHistory?.length === 0 ? (
              ...
              <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                {selectedPolicy.premiumHistory?.map((hist) => (
                  <div
                    key={hist.id}
                    className="border border-border p-2.5 rounded-lg text-[10px] space-y-1 bg-accent/20"
                  >
                    ...
                    <div className="flex justify-between text-muted-foreground">
                      <span>
                        Paid: {new Date(hist.paidDate).toLocaleString()}
                      </span>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, sumAssured: e.target.value })
                    }
                    className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...form, premiumMode: e.target.value })
                    }
                    className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                  >
