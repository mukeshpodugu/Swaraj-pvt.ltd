...
export default function Settings() {
  ...
  async function loadLogs() {
    try {
      ...
      const res = await axios.get("/api/settings/logs", {
        params: {
          type: logsTab === "notifications" ? "notifications" : "audit",
        },
      });
  ...
  const handleExportBackup = () => {
    window.open(
      `${axios.defaults.baseURL || "http://localhost:5000"}/api/settings/backup/export`,
      "_blank",
    );
  };
  ...
  const handleRestoreBackup = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        await axios.post("/api/settings/backup/restore", {
          backupData: parsed,
        });
        triggerToast(
          "Database successfully restored from JSON backup. Refreshing page.",
        );
        window.location.reload();
      } catch (err) {
        alert(
          "Restore failed. Invalid JSON structure or DB constraints rejected import.",
        );
      } finally {
        setActionLoading(false);
      }
    ...
    <div className="space-y-6 fade-in text-xs sm:text-sm relative">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground font-medium">
          Configure company interest rates, manage backups, and review operator
          audit trails.
        </p>
      </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        basePenaltyRate: parseFloat(e.target.value),
                      })
                    }
                    className="w-full bg-accent/40 border border-border rounded-xl py-2 px-3 focus:outline-none"
                  />
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4">
            ...
            <p className="text-[10px] text-muted-foreground leading-normal">
              Generate a snapshot JSON container containing all users, active
              loans, chits, and logs. Snapshot files can be imported directly to
              restore database coordinates.
            </p>
              ...
              <div className="max-h-72 overflow-y-auto space-y-3 pr-1 text-[10px]">
                {logsTab === "audit"
                  ? systemLogs.map((log) => (
                      <div
                        key={log.id}
                        className="border-b border-border/60 pb-2.5 last:border-b-0 space-y-1"
                      >
                        ...
                        {log.user && (
                          <span className="text-[8px] bg-accent px-1.5 py-0.5 rounded font-medium text-muted-foreground">
                            Operator: {log.user.fullName} ({log.user.role})
                          </span>
                        )}
                      </div>
                    ))
                  : systemLogs.map((log) => (
                      ...
                      >
                        <div className="flex justify-between font-semibold">
                          ...
                          <span
                            className={`px-1.5 rounded-[4px] text-[8px] font-bold ${
                              log.status === "SENT"
