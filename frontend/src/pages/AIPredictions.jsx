...
export default function AIPredictions() {
  ...
  async function loadDropdowns() {
    try {
      ...
      setLoans(loansRes.data.filter((l) => l.status === "DISBURSED"));
    } catch (err) {
  ...
  const formatFeatureData = (featRecord) => {
    ...
    <div className="space-y-6 fade-in text-xs sm:text-sm">
      ...
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight">
          AI Predictive Credit Center
        </h1>
        <p className="text-xs text-muted-foreground font-medium">
          Underwrite loan approvals, run default calculators, and inspect
          confidence variables weight metrics.
        </p>
      </div>
        ...
        <div className="lg:col-span-5 bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4">
          {activeTab === "approval" ? (
            ...
            <form onSubmit={handleApprovalSubmit} className="space-y-4 text-xs">
              <h3 className="font-extrabold text-sm uppercase tracking-wider text-muted-foreground border-l-2 border-primary pl-2 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setApprovalForm({
                        ...approvalForm,
                        requestedPrincipal: e.target.value,
                      })
                    }
                    className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                  />
              ...
              <div className="flex items-center gap-2 p-1.5">
                ...
                <label
                  htmlFor="collateralCheck"
                  className="font-bold text-muted-foreground cursor-pointer select-none"
                >
              ...
              >
                ...
                <span>
                  {predictLoading
                    ? "Calculating underwriting matrix..."
                    : "Execute Predictive AI Underwriter"}
                </span>
              </button>
            </form>
          ) : (
            ...
            <form onSubmit={handleDefaultSubmit} className="space-y-4 text-xs">
              ...
              <div>
                <label className="font-bold text-muted-foreground block mb-1">
                  Select Disbursed Loan File
                </label>
                <select
                  value={defaultForm.loanId}
                  onChange={(e) =>
                    setDefaultForm({ ...defaultForm, loanId: e.target.value })
                  }
                  required
                  className="w-full bg-accent/40 border border-border rounded-xl py-2.5 px-3 focus:outline-none"
                >
                  ...
                  {loans.map((l) => (
                    <option key={l.id} value={l.id}>
                      {l.customer.user.fullName} &bull; {l.loanType} (
              ...
              >
                ...
                <span>
                  {predictLoading
                    ...
                    : "Evaluate Delinquency Default Risk"}
                </span>
              </button>
            </form>
          )}
          ...
          {!approvalResult && !defaultResult && (
            <div className="flex-1 flex flex-col items-center justify-center text-center text-muted-foreground space-y-3">
              ...
              <p className="text-sm font-semibold">
                Ready to Evaluate borrower risk
              </p>
              <p className="text-xs max-w-sm">
                Provide details in the left settings panel and click Execute to
                view underwriting predictions.
              </p>
            </div>
          )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="border border-border p-4 rounded-xl text-center space-y-1 bg-accent/10">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    Confidence: {approvalResult.confidenceScore}%
                  </span>
                </div>
                ...
                <div className="border border-border p-4 rounded-xl text-center space-y-1 bg-accent/10">
                  ...
                  <h4
                    className={`text-xl font-extrabold mt-1.5 uppercase ${
                ...
                <div className="border border-border p-4 rounded-xl text-center space-y-1 bg-accent/10">
                  ...
                  <h4 className="text-3xl font-extrabold text-foreground">
                    {approvalResult.customerCreditScore}
                  </h4>
                  <span className="text-[10px] text-muted-foreground">
                    Rating: {approvalResult.customerCreditRating}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="h-44 border border-border rounded-xl p-2 bg-accent/5">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={formatFeatureData(
                          approvalResult.featureImportance,
                        )}
                        layout="vertical"
                      >
                        <CartesianGrid
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-accent/10 border border-border p-4 rounded-xl space-y-2.5">
                    {approvalResult.recommendations.map((rec, i) => (
                      <p
                        key={i}
                        className="text-xs text-muted-foreground leading-normal flex items-start gap-2"
                      >
            ...
            <div className="space-y-6 fade-in text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="border border-border p-4 rounded-xl text-center space-y-1 bg-accent/10">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    Confidence: {defaultResult.confidenceScore}%
                  </span>
                </div>
                ...
                <div className="border border-border p-4 rounded-xl text-center space-y-1 bg-accent/10">
                  ...
                  <h4
                    className={`text-2xl font-extrabold mt-1 uppercase ${
                      defaultResult.riskLevel === "HIGH"
                        ? "text-destructive"
                        : defaultResult.riskLevel === "MEDIUM"
                          ? "text-orange-500"
                          : "text-green-500"
                    }`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="h-44 border border-border rounded-xl p-2 bg-accent/5">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={formatFeatureData(
                          defaultResult.featureImportance,
                        )}
                        layout="vertical"
                      >
                        <CartesianGrid
                        ...
                        <Bar dataKey="Weight" fill="#ef4444">
                          {formatFeatureData(
                            ...
                            <Cell
                              key={`cell-${index}`}
                              fill={entry.Weight >= 15 ? "#ef4444" : "#f59e0b"}
                            />
                          ))}
                        </Bar>
                      </BarChart>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-accent/10 border border-border p-4 rounded-xl space-y-2.5">
                    {defaultResult.recommendations.map((rec, i) => (
                      <p
                        key={i}
                        className="text-xs text-muted-foreground leading-normal flex items-start gap-2"
                      >
      ...
      <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h4 className="font-bold text-sm">
            Historical AI Underwriting Runs Logs
          </h4>
        </div>
