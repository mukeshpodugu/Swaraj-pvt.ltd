...
export default function CustomerPortal() {
  ...
  const [allChitGroups, setAllChitGroups] = useState([]);
    ...
    try {
      ...
      if (!profileId && user?.email) {
        const searchRes = await axios.get(
          `/api/customers?search=${encodeURIComponent(user.email)}`,
        );
  ...
  const handlePayEmi = async (loanId, emiId, defaultAmount) => {
    try {
      ...
      alert(
        "EMI Payment received successfully! Receipt has been registered in the system.",
      );
  ...
  const handlePayChitInstallment = async (
    chitGroupId,
    instId,
    defaultAmount,
  ) => {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      setActionLoading(true);
      await axios.put(
        `/api/customers/${user.customerProfileId}`,
        editProfileForm,
      );
  ...
  const handleApplyLoan = async (e) => {
    ...
    try {
      ...
      await axios.post("/api/loans/apply", {
        ...
        collateralDescription:
          loanForm.collateralDescription || "None Provided",
        guarantorName: loanForm.guarantorName || "None",
        guarantorPhone: loanForm.guarantorPhone || "None",
        collateralUrl: loanForm.collateralUrl || "",
      });
      alert(
        "Loan application and collateral details submitted successfully to Swaraj operators!",
      );
  ...
  const handleBuyLic = async (e) => {
    ...
    try {
      ...
      alert(
        `LIC Policy ${policyNo} registered successfully! You can pay premium renewals on the portal.`,
      );
  ...
  const handleJoinChitGroup = async (groupId) => {
    try {
      setActionLoading(true);
      ...
      alert(
        "Successfully joined the Chit Fund Group! A subscriber slot has been allocated.",
      );
      await loadCustomerData();
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center font-sans p-6">
        <div className="text-center max-w-sm space-y-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl">
          <AlertCircle
            size={48}
            className="mx-auto text-destructive animate-pulse"
          />
          <h2 className="text-lg font-bold text-white">
            Profile Configuration Error
          </h2>
          <p className="text-xs text-slate-400 leading-relaxed">
            We were unable to load your customer profile context. Please contact
            support or try logging out and logging back in.
          </p>
          <button
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between font-sans w-full relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              <img
                src="/logo.jpg"
                alt="Swaraj Private Limited Logo"
                className="h-8 rounded bg-white p-0.5 object-contain"
              />
              <div>
                <h1 className="text-[10px] font-extrabold text-white tracking-tight uppercase">
                  Swaraj Pvt. Ltd.
                </h1>
      ...
      <main className="flex-1 overflow-y-auto w-full max-w-6xl mx-auto p-4 md:p-6 space-y-6 pb-24 md:pb-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="p-5 bg-gradient-to-r from-blue-900 to-indigo-900 rounded-2xl relative overflow-hidden space-y-2">
                  ...
                  <p className="text-[10px] text-slate-300 leading-normal">
                    Manage your active chits, pay EMI installments, or verify
                    nominee records.
                  </p>
                  <div className="flex items-center gap-1.5 mt-2 bg-black/30 w-fit px-2.5 py-1 rounded text-[8px] font-bold uppercase">
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${profile.kycStatus === "VERIFIED" ? "bg-green-500" : "bg-orange-500"}`}
                    ></span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-slate-900 border border-slate-800/80 p-4 rounded-xl space-y-1">
                    <span className="text-[9px] text-slate-400 block uppercase font-bold tracking-wider">
                      Active Chit Holdings
                    </span>
                    <span className="text-base font-extrabold text-white">
                      Rs.{" "}
                      {profile.chits
                        .reduce((sum, c) => sum + c.chitGroup.groupValue, 0)
                        .toLocaleString()}
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="bg-slate-900 border border-slate-800/80 p-4 rounded-xl space-y-2 text-xs">
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-slate-200">
                        LIC: {profile.licPolicies[0].planName}
                      </span>
                      <span className="text-[10px] text-orange-400">
                        Due:{" "}
                        {new Date(
                          profile.licPolicies[0].dueDate,
                        ).toLocaleDateString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-[10px] text-slate-400">
                      <span>
                        Policy No: {profile.licPolicies[0].policyNumber}
                      </span>
                      <span>
                        Premium: Rs. {profile.licPolicies[0].premiumAmount}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-slate-900 border border-slate-800/80 p-4 rounded-xl space-y-3 text-xs">
                  ...
                  {showLicModal ? (
                    <form
                      onSubmit={handleBuyLic}
                      className="space-y-3 mt-2 text-[10px]"
                    >
                      <div>
                        <label className="font-semibold text-slate-400 block mb-1 text-[9px]">
                          LIC Plan Type
                        </label>
                        <select
                          value={licForm.planName}
                          onChange={(e) =>
                            setLicForm({ ...licForm, planName: e.target.value })
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-white"
                        >
                          <option value="Jeevan Labh (836)">
                            Jeevan Labh (836)
                          </option>
                          <option value="Jeevan Anand (915)">
                            Jeevan Anand (915)
                          </option>
                          <option value="Jeevan Umang (945)">
                            Jeevan Umang (945)
                          </option>
                          <option value="Tech Term Insurance Plan">
                            Tech Term Insurance
                          </option>
                        </select>
                      </div>
                      ...
                      >
                        {actionLoading
                          ? "Processing..."
                          : "Confirm Policy Registration"}
                      </button>
                    </form>
                  ) : (
                    <p className="text-[10px] text-slate-400">
                      Explore Swaraj's LIC insurance plans and register a policy
                ...
                <div className="space-y-2.5">
                  <h3 className="font-extrabold text-[10px] text-slate-400 uppercase tracking-widest border-l-2 border-primary pl-2">
                    Recent Notifications
                  </h3>

                  {timeline.filter((e) => e.type.startsWith("NOTIF_"))
                    .length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {timeline
                        .filter((e) => e.type.startsWith("NOTIF_"))
                        .slice(0, 3)
                        .map((event, idx) => (
                          <div
                            key={idx}
                            className="bg-slate-900/50 border border-slate-900 p-3 rounded-lg text-[10px] flex gap-2"
                          >
          ...
          <div className="space-y-4 fade-in">
            ...
            {showLoanModal && (
              <form
                ...
                className="bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-4 text-xs max-w-lg"
              >
                <h3 className="font-bold text-white text-xs border-b border-slate-800 pb-1.5">
                  New Loan Request Form
                </h3>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="font-semibold text-slate-400 block mb-1 text-[10px]">
                      Loan Type
                    </label>
                    <select
                ...
                <div>
                  <label className="font-semibold text-slate-400 block mb-1 text-[10px]">
                    Collateral Document URL / File Reference
                  </label>
                  <input
                    type="text"
                  ...
                  >
                    <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                      <div>
                        ...
                        <span className="text-[9px] text-slate-500 block">
                          File ID: {loan.id.substring(0, 8).toUpperCase()}
                        </span>
                      </div>
                      <div className="text-right">
                        ...
                        <span
                          className={`text-[8px] px-1.5 py-0.5 rounded font-bold uppercase inline-block mt-0.5 ${
                            loan.collateralStatus === "VERIFIED"
                              ? "bg-green-500/10 text-green-500"
                              : loan.collateralStatus === "REJECTED"
                                ? "bg-red-500/10 text-red-500"
                                : "bg-yellow-500/10 text-yellow-500"
                          }`}
                        >
                          Collateral: {loan.collateralStatus}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-950/40 p-2.5 rounded-lg text-[10px] space-y-1">
                      ...
                      <p className="text-slate-300">
                        Desc: {loan.collateralDescription || "Not Provided"}
                      </p>
                      {loan.collateralUrl && (
                    ...
                    <div className="space-y-2 text-[10px]">
                      <span className="font-bold text-slate-400">
                        Monthly Installment Checklist
                      </span>

                      {loan.emiInstallments?.length === 0 ? (
                        <p className="text-[9px] text-slate-500">
                          Schedule loading or not disbursed yet.
                        </p>
                      ) : (
                        <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                          {loan.emiInstallments?.map((emi) => (
                            <div
                              key={emi.id}
                              className="flex justify-between items-center bg-slate-950/60 p-2.5 rounded-lg border border-slate-900"
                            >
                              <div>
                                <p className="font-semibold text-slate-300">
                                  EMI #{emi.installmentNo} &bull; Rs.{" "}
                                  {emi.totalAmount}
                                </p>
                              ...
                              {emi.status === "PAID" ? (
                                <span className="text-[8px] bg-green-500/10 text-green-500 px-2 py-0.5 rounded font-extrabold">
                                  PAID
                                </span>
                              ) : (
                                <button
        ...
        {activeTab === "chits" && (
          <div className="space-y-4 fade-in">
            <h2 className="text-sm font-extrabold text-white">
              Your Chit Fund Subscriptions
            </h2>

            {profile.chits.length === 0 ? (
              <p className="text-xs text-slate-500">
                You are not enrolled in any chit groups.
              </p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {profile.chits.map((membership) => {
                  const group = membership.chitGroup;
                  const groupInstallments =
                    group.installments?.filter(
                      (inst) => inst.customerId === profile.id,
                    ) || [];
                  return (
                    <div
                      key={membership.id}
                      ...
                      <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                        <div>
                          ...
                          <span className="text-[9px] text-slate-400">
                            Allocated Slot Slot #{membership.slotNumber}
                          </span>
                        </div>
                        <span className="text-xs font-bold text-primary">
                          Rs. {group.groupValue.toLocaleString()}
                        </span>
                      </div>
                          ...
                          <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                            {groupInstallments.map((inst) => (
                              ...
                              >
                                <div>
                                  <p className="font-semibold text-slate-300">
                                    Round #{inst.installmentNo} &bull; Rs.{" "}
                                    {inst.dueAmount}
                                  </p>
                                  <span className="text-[8px] text-slate-500">
                                    Due:{" "}
            ...
            <div className="space-y-4 pt-6 border-t border-slate-800/80">
              <h3 className="text-xs font-extrabold text-slate-400 uppercase tracking-widest pl-2 border-l-2 border-primary">
                Available Chits to Join
              </h3>

              {(() => {
                const enrolledGroupIds = profile.chits.map(
                  (c) => c.chitGroupId,
                );
                const joinableGroups = allChitGroups.filter(
                  (g) =>
                    !enrolledGroupIds.includes(g.id) && g.status === "ACTIVE",
                );

                if (joinableGroups.length === 0) {
                  return (
                    <p className="text-[10px] text-slate-500">
                      No new active chit groups available for enrollment.
                    </p>
                  );
                }
                ...
                return (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {joinableGroups.map((group) => (
                      <div
                        key={group.id}
                        className="bg-slate-900 border border-slate-800/80 p-4 rounded-xl flex justify-between items-center text-xs"
                      >
                        <div>
                          ...
                          <p className="text-[9px] text-slate-500 mt-1">
                            Value: Rs. {group.groupValue.toLocaleString()}{" "}
                            &bull; Duration: {group.durationMonths} Mo
                          </p>
                        </div>
                        <button
                          type="button"
          ...
          <div className="space-y-4 fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2 text-[10px]">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-white">
                      Rs. {profile.monthlyIncome.toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <form
                ...
                className="bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-4 text-xs"
              >
                <span className="font-bold text-slate-400 block border-b border-slate-800 pb-1.5">
                  Update Information coordinates
                </span>
