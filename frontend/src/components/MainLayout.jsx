...
export default function MainLayout({ children }) {
  ...
  const navItems = [
    {
      ...
      roles: ["SUPER_ADMIN", "ADMIN", "STAFF", "AGENT"],
    },
    {
      name: "Customers CRM",
      path: "/customers",
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground transition-colors duration-300">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col bg-[#0b192c] text-slate-300 w-64 transform ${
        ...
        <div className="p-4 border-b border-slate-800 bg-slate-950/20">
          <Link to="/dashboard" className="flex flex-col items-center gap-2">
            <img
              ...
              className="h-14 rounded-lg object-contain bg-white p-0.5"
            />
            <div className="text-center">
              <h2 className="font-extrabold text-white text-xs tracking-tight uppercase">
                Swaraj Pvt. Ltd.
              </h2>
              <span className="text-[8px] text-slate-500 block tracking-widest uppercase">
                Finance Platform
              </span>
            </div>
          </Link>
                  ...
                  <div className="space-y-3">
                    <div className="p-2 hover:bg-accent/40 rounded-lg cursor-pointer">
                      <p className="font-semibold text-xs">
                        LIC policy premium due
                      </p>
                      <span className="text-[10px] text-muted-foreground">
                        LIC-778899 premium due in 15 days.
                      </span>
                    </div>
                    <div className="p-2 hover:bg-accent/40 rounded-lg cursor-pointer">
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        Installment #3 for customer Amit Patel is pending.
                      </span>
                    </div>
                  </div>
                </div>
