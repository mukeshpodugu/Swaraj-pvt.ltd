export class AIService {
   ...
   */
  static predictLoanApproval(input) {
    const monthlyRate = input.interestRate / 100 / 12;
    const emi =
      (input.requestedPrincipal *
        monthlyRate *
        Math.pow(1 + monthlyRate, input.tenorMonths)) /
    ...
    const confidenceScore = Math.max(
      ...
      Math.min(
        98,
        50 + input.creditScore / 18 + (input.hasCollateral ? 10 : 0),
      ),
    );
    ...
    return {
      ...
      recommendations: this.getLoanRecommendations(
        dtiRatio,
        input.creditScore,
        input.hasCollateral,
        input.latePaymentsCount,
      ),
    };
  }
   ...
   */
  static predictDefaultRisk(input) {
    const principalPaidRatio =
      (input.originalPrincipal - input.outstandingPrincipal) /
    ...
    const confidenceScore = Math.max(
      ...
      Math.min(
        99,
        75 +
          (input.daysPastDue > 0 ? 15 : 0) +
          (input.creditScore > 700 ? 5 : 0),
      ),
    );
   ...
   */
  static calculateCustomerCreditScore(
    income,
    totalOutstandingDebt,
    latePayments,
    successfulEMIReviews,
  ) {
  ...
  static getLoanRecommendations(dti, credit, collateral, late) {
    const list = [];
    if (dti > 45) {
      list.push(
        "Reduce the requested principal amount to bring DTI ratio under 45%.",
      );
    }
    if (credit < 600) {
      list.push(
        "Improve credit score by paying outstanding credit cards or previous bills.",
      );
    }
    ...
    if (late > 1) {
      list.push(
        "Ensure consecutive on-time payments for 3-6 months to demonstrate behavioral recovery.",
      );
    }
    if (list.length === 0) {
      list.push(
        "Borrower profiles meet all benchmarks. Recommended for fast-track disbursal.",
      );
    }
    return list;
  }

  static getDefaultRecommendations(risk, dpd, outstanding) {
    const list = [];
    if (dpd > 30) {
      list.push(
        "Initiate restructuring dialogue with borrower for loan consolidation.",
      );
    }
    if (risk > 50) {
      list.push(
        "Halt further credit approvals, chits, and LIC renewals under company-financed agent schemes.",
      );
      list.push(
        "Issue formal legal reminder under Negotiable Instruments Act (if applicable).",
      );
    }
    if (outstanding > 100000 && risk > 30) {
      list.push(
        "Contact guarantor immediately to inform them of payment delays.",
      );
    }
    if (list.length === 0) {
