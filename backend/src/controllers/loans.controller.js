...
export class LoansController {
  ...
  static async applyForLoan(req, res) {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Required loan parameters missing." });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error processing loan application." });
    }
  }
  ...
  static async approveLoan(req, res) {
    ...
    if (
      !status ||
      ![LoanStatus.APPROVED, LoanStatus.REJECTED].includes(status)
    ) {
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            error:
              "Collateral must be VERIFIED before approving and sanctioning this loan.",
          });
      }
      ...
      return res
        .status(500)
        .json({ error: "Server error updating loan status." });
    }
  }
   ...
   */
  static async verifyCollateral(req, res) {
    const { id } = req.params;
    ...
    if (!status || !["VERIFIED", "REJECTED"].includes(status)) {
      return res
        .status(400)
        .json({ error: "Invalid collateral status option." });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({
          error: "Server error updating collateral verification status.",
        });
    }
  }
  ...
  static async disburseLoan(req, res) {
    ...
    try {
      ...
      if (loan.status !== LoanStatus.APPROVED) {
        return res
          .status(400)
          .json({ error: "Loan must be APPROVED before disbursal." });
      }
      ...
      return res
        .status(500)
        .json({ error: "Server error processing loan disbursal." });
    }
  }
  ...
  static async payEMI(req, res) {
    ...
    try {
      ...
      if (emi.status === PaymentStatus.PAID) {
        return res
          .status(400)
          .json({ error: "This EMI has already been paid." });
      }
      ...
      return res
        .status(500)
        .json({ error: "Server error processing EMI payment." });
    }
  }
   ...
   */
  static async makePartPayment(req, res) {
    ...
    if (!amount || parseFloat(amount) <= 0) {
      return res
        .status(400)
        .json({ error: "Valid part payment amount is required." });
    }
    ...
    try {
      ...
      return res.json({
        message:
          "Part-payment successfully processed. Loan outstanding balance reduced.",
        partPayment,
      ...
      return res
        .status(500)
        .json({ error: "Server error processing part-payment." });
    }
  }
  ...
  static async getAllLoans(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error retrieving loans list." });
    }
  }
}
