...
export class ReportsController {
   ...
   */
  static async sendReportFile(res, data, format, filename) {
    try {
      if (format === "pdf") {
      ...
      return res
        .status(500)
        .json({ error: "Failed to compile and download report file." });
    }
  }
   ...
   * Customers Report
   */
  static async downloadCustomersReport(req, res) {
    ...
    try {
      ...
      return ReportsController.sendReportFile(
        res,
        data,
        format,
        "Swaraj_Customers_Report",
      );
    } catch (err) {
   ...
   * Loans Report
   */
  static async downloadLoansReport(req, res) {
   ...
   * Chit Funds Report
   */
  static async downloadChitsReport(req, res) {
   ...
   * LIC Policies Report
   */
  static async downloadLICReport(req, res) {
    const { format = "csv", status } = req.query;
   ...
   * Defaulters Report
   */
  static async downloadDefaultersReport(req, res) {
    ...
    try {
      ...
      const overdueEMIs = await prisma.eMIInstallment.findMany({
        ...
        include: {
          loan: { include: { customer: { include: { user: true } } } },
        },
      });
  ...
  static async downloadCashFlowReport(req, res) {
    ...
    try {
      ...
      const loanInterestIncome = paidEMIs.reduce(
        (sum, e) => sum + e.interestAmount,
        0,
      );
      ...
      const chitCommissions = auctions.reduce(
        (sum, a) =>
          sum + a.chitGroup.groupValue * (a.chitGroup.commissionRate / 100),
        0,
      );
      ...
      const licCommissions = licHistory.reduce(
        (sum, p) => sum + p.agentCommission,
        0,
      );
      ...
      const loanDisbursalExpense = disbursedLoans.reduce(
      ...
      const reportRows = [
        {
          item: "Lending Operations Interest Collected",
          type: "INCOME",
          amount: loanInterestIncome,
        },
        {
          item: "Chit Group Auction Administration Commissions",
          type: "INCOME",
          amount: chitCommissions,
        },
        {
          item: "LIC Policy Sales Commission Earnings",
          type: "INCOME",
          amount: licCommissions,
        },
        {
          item: "Capital Loan Disbursals Capital Outflow",
          type: "EXPENSE",
          amount: loanDisbursalExpense,
        },
      ];
   ...
   */
  static async triggerOverdueReminders(req, res) {
    try {
      const today = new Date();
      let remindersSent = 0;
      ...
      for (const emi of overdueEMIs) {
        try {
          ...
          console.error(
            `Failed to send overdue EMI email to customer ${emi.loan.customerId}`,
            err,
          );
        }
      }
      ...
      for (const inst of overdueChits) {
        try {
          ...
          console.error(
            `Failed to send overdue chit email to customer ${inst.customerId}`,
            err,
          );
        }
      }
      ...
      for (const policy of overdueLICs) {
        try {
          ...
          console.error(
            `Failed to send overdue LIC email to customer ${policy.customerId}`,
            err,
          );
        }
      }
      ...
      return res
        .status(500)
        .json({ error: "Server error processing overdue reminder scan." });
    }
  }
   ...
   */
  static async downloadMonthlyStatusReport(req, res) {
    ...
    try {
        ...
        reportRows.push({
          ...
          paidDate:
            emi.status === "PAID" && emi.paidDate
              ? emi.paidDate.toLocaleDateString()
              : "-",
        });
      });
        ...
        reportRows.push({
          ...
          paidDate:
            inst.status === "PAID" && inst.paidDate
              ? inst.paidDate.toLocaleDateString()
              : "-",
        });
      });

      // Add Paid LIC Premiums
      paidLICHistory.forEach((hist) => {
        reportRows.push({
      ...
      activeLics.forEach((policy) => {
        const hasPaidThisPeriod = paidLICHistory.some(
          (h) => h.policyId === policy.id,
        );
      ...
      const totalPaid = reportRows
        .filter((r) => r.status === "PAID")
        .reduce((sum, row) => sum + row.amount, 0);
      const totalUnpaid = reportRows
        .filter((r) => r.status !== "PAID")
        .reduce((sum, row) => sum + row.amount, 0);
      ...
      return ReportsController.sendReportFile(
        ...
        data,
        format,
        `Swaraj_Monthly_Collections_Report`,
      );
    } catch (err) {
      ...
      return res
        .status(500)
        .json({
          error: "Server error generating monthly collections status report.",
        });
    }
  }
}
