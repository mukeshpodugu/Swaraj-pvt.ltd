...
export class CustomersController {
  ...
  static async getCustomers(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error retrieving customers." });
    }
  }
  ...
  static async getCustomerById(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error retrieving customer details." });
    }
  }
  ...
  static async verifyKYC(req, res) {
    ...
    if (!status || !Object.values(KYCStatus).includes(status)) {
      return res
        .status(400)
        .json({ error: "Valid KYC Status (VERIFIED / REJECTED) is required." });
    }
   ...
   */
  static async getCustomerTimeline(req, res) {
    const { id } = req.params;
    ...
    try {
          ...
          chits: {
            include: {
              chitGroup: {
                include: {
                  auctions: true,
                  installments: { where: { customerId: id } },
                },
              },
            },
          },
      ...
      if (profile.verifiedAt) {
        timeline.push({
          ...
          title: "KYC Approved",
          description:
            "Aadhaar, PAN, and Photo KYC documents verified by Compliance Officer.",
        });
      }
      ...
      return res
        .status(500)
        .json({ error: "Server error generating timeline logs." });
    }
  }
}
