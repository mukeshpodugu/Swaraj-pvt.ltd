...
export class AIController {
   ...
   */
  static async predictApproval(req, res) {
    const {
      customerId,
      requestedPrincipal,
      tenorMonths,
    ...
    if (!customerId || !requestedPrincipal || !tenorMonths || !interestRate) {
      return res
        .status(400)
        .json({
          error: "Missing input parameters for loan approval prediction.",
        });
    }
    ...
    try {
      ...
      const profile = await prisma.customerProfile.findUnique({
        ...
        include: {
          ...
          chits: {
            include: {
              chitGroup: {
                include: { installments: { where: { customerId } } },
              },
            },
          },
        },
      ...
      profile.loans.forEach((loan) => {
        loan.emiInstallments.forEach((emi) => {
          if (
            emi.status === "OVERDUE" ||
            (emi.status === "UNPAID" && new Date() > emi.dueDate)
          ) {
            latePayments++;
          }
        });
      ...
      profile.chits.forEach((membership) => {
        membership.chitGroup.installments.forEach((inst) => {
          if (
            inst.status === "OVERDUE" ||
            (inst.status === "UNPAID" && new Date() > inst.dueDate)
          ) {
            latePayments++;
          }
        });
      ...
      const creditEval = AIService.calculateCustomerCreditScore(
        ...
        latePayments,
        profile.loans.reduce(
          (sum, l) =>
            sum + l.emiInstallments.filter((e) => e.status === "PAID").length,
          0,
        ),
      );
      ...
      return res
        .status(500)
        .json({ error: "Server error running AI underwriter." });
    }
  }
   ...
   */
  static async predictDefault(req, res) {
    ...
    try {
        ...
        include: {
          ...
          chits: {
            include: {
              chitGroup: {
                include: {
                  installments: { where: { customerId: loan.customerId } },
                },
              },
            },
          },
      ...
      loan.emiInstallments.forEach((emi) => {
        if (
          ...
          const diffTime = Math.abs(
            new Date().getTime() - emi.dueDate.getTime(),
          );
      ...
      return res
        .status(500)
        .json({ error: "Server error running AI default assessment." });
    }
  }
  ...
  static async getCustomerScore(req, res) {
    ...
    try {
      const profile = await prisma.customerProfile.findUnique({
        ...
        include: {
          ...
          chits: {
            include: {
              chitGroup: {
                include: { installments: { where: { customerId: id } } },
              },
            },
          },
        },
      ...
      const totalDebt = profile.loans.reduce(
      ...
      return res
        .status(500)
        .json({ error: "Server error evaluating borrower score." });
    }
  }
