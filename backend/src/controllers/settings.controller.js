...
export class SettingsController {
  ...
  static async getSettings(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error retrieving settings." });
    }
  }
  ...
  static async updateSettings(req, res) {
    ...
    try {
      ...
      const dataPayload = {
        ...
        ...(basePenaltyRate !== undefined && {
          basePenaltyRate: parseFloat(basePenaltyRate),
        }),
  ...
  static async getSystemLogs(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error retrieving system log history." });
    }
  }
  ...
  static async exportBackup(req, res) {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="Swaraj_DB_Backup_${Date.now()}.json"`,
      );
      ...
      return res
        .status(500)
        .json({ error: "Server error compiling database JSON backup." });
    }
  }
  ...
  static async restoreBackup(req, res) {
    ...
    if (!backupData || !backupData.tables) {
      return res
        .status(400)
        .json({ error: "Valid database JSON backup data is required." });
    }
    ...
    try {
      ...
      await prisma.$executeRawUnsafe(
        `TRUNCATE TABLE "NotificationLog" CASCADE;`,
      );
      await prisma.$executeRawUnsafe(`TRUNCATE TABLE "AuditLog" CASCADE;`);
      await prisma.$executeRawUnsafe(
        `TRUNCATE TABLE "AIPredictionLog" CASCADE;`,
      );
      await prisma.$executeRawUnsafe(
        `TRUNCATE TABLE "LICPremiumHistory" CASCADE;`,
      );
      await prisma.$executeRawUnsafe(`TRUNCATE TABLE "LICPolicy" CASCADE;`);
      await prisma.$executeRawUnsafe(
        `TRUNCATE TABLE "ChitInstallment" CASCADE;`,
      );
      ...
      await prisma.$executeRawUnsafe(
        `TRUNCATE TABLE "EMIInstallment" CASCADE;`,
      );
      await prisma.$executeRawUnsafe(`TRUNCATE TABLE "Loan" CASCADE;`);
      await prisma.$executeRawUnsafe(
        `TRUNCATE TABLE "CustomerProfile" CASCADE;`,
      );
      await prisma.$executeRawUnsafe(`TRUNCATE TABLE "User" CASCADE;`);
      await prisma.$executeRawUnsafe(
        `TRUNCATE TABLE "CompanySetting" CASCADE;`,
      );
      ...
      return res.json({
        message: "Database successfully restored from JSON backup container.",
      });
    } catch (err) {
      ...
      return res
        ...
        .json({
          error:
            "Server error restoring tables. Database constraints may have failed.",
        });
    }
  }
}
