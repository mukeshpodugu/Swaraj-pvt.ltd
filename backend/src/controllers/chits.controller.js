...
export class ChitsController {
   ...
   */
  static async createChitGroup(req, res) {
    const {
      name,
      groupValue,
      durationMonths,
    ...
    if (!name || !groupValue || !durationMonths || !maxMembers || !startDate) {
      return res
        .status(400)
        .json({ error: "Required chit parameters missing." });
    }
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: "Group creation failed. Name may already be taken." });
    }
  }
   ...
   */
  static async joinChitGroup(req, res) {
    ...
    if (!customerId || !slotNumber) {
      return res
        .status(400)
        .json({ error: "Customer ID and slot number are required." });
    }
    ...
    try {
      ...
      if (isSlotTaken) {
        return res
          .status(400)
          .json({ error: `Slot number ${slot} is already occupied.` });
      }
      ...
      if (isMember) {
        return res
          .status(400)
          .json({ error: "Customer is already enrolled in this Chit group." });
      }
      ...
      return res
        .status(500)
        .json({ error: "Server error enrolling customer in Chit group." });
    }
  }
   ...
   */
  static async conductAuction(req, res) {
    const { id } = req.params; // Chit Group ID
    ...
    if (!installmentNo || !winningBidderId || bidAmount === undefined) {
      return res
        .status(400)
        .json({ error: "Required auction resolution details missing." });
    }
    ...
    try {
      const group = await prisma.chitGroup.findUnique({
        where: { id },
        include: {
          members: { include: { customer: { include: { user: true } } } },
        },
      });
      ...
      if (existingAuction) {
        return res
          .status(400)
          .json({
            error: `Auction for installment #${instNo} has already been resolved.`,
          });
      }
      ...
      const netMonthlyContribution =
        group.monthlyContribution - dividendPerMember;
      ...
      // Notify winner
      const winnerMember = group.members.find(
        (m) => m.customerId === winningBidderId,
      );
      if (winnerMember) {
        await MailService.sendEmail(
          winningBidderId,
      ...
      for (const member of group.members) {
        if (member.customerId !== winningBidderId) {
          try {
            ...
            console.error(
              `Failed to send next due email to chit subscriber ${member.customerId}`,
              mailErr,
            );
          }
        }
      ...
      return res
        .status(500)
        .json({ error: "Server error conducting chit auction." });
    }
  }
  ...
  static async payInstallment(req, res) {
    ...
    try {
      ...
      if (!installment) {
        return res
          .status(404)
          .json({ error: "Chit installment record not found." });
      }

      if (installment.status === PaymentStatus.PAID) {
        return res
          .status(400)
          .json({ error: "Installment is already fully paid." });
      }
      ...
      if (isPastDue) {
        ...
        penalty =
          Math.round(installment.dueAmount * (penaltyRate / 100) * 100) / 100;
      }
      ...
      return res
        .status(500)
        .json({ error: "Server error processing chit payment." });
    }
  }
   ...
   */
  static async getChitGroups(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error retrieving chit groups." });
    }
  }
}
